...
from classes import Animais, Parede, Mago, Rio, Projectile, Projectile_hud, Dragao, functions, Menu, Coletaveis, Animacao
    ...
    def create_bridges(self) -> None:
        ...
        while not self.ponte:
            ...
            if (bloco.x - 100, bloco.y) in self.mapa.keys() and (bloco.x + 100, bloco.y) in self.mapa.keys():
                self.ponte = True
                if (bloco.x - 50, bloco.y + 50) not in self.mapa.keys() or (bloco.x + 100, bloco.y + 50) not in self.mapa.keys():
                    self.ponte = False
                elif self.mapa[(bloco.x - 50, bloco.y)] in proibidos or self.mapa[(bloco.x - 50, bloco.y + 50)] in proibidos or self.mapa[(bloco.x + 100, bloco.y)] in proibidos or self.mapa[(bloco.x + 100, bloco.y + 50)] in proibidos:
                    self.ponte = False
                for rio in Rio.rios:
